...
class SizesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create size" do
    assert_difference('Size.count') do
      post sizes_url, params: { size: { name: @size.name, status: @size.status, text: @size.text } }
    end
  ...
  test "should update size" do
    patch size_url(@size), params: { size: { name: @size.name, status: @size.status, text: @size.text } }
